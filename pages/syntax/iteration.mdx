...
We use `forEach` to call a function for each element of an array. <Details>The function is also passed the element, the element's index, and the array itself.</Details>

We'll generally use this instead of `for ... of`, unless we need to exit the loop early<Details> (i.e. `break` doesn't exist in `forEach` loops)</Details>.
